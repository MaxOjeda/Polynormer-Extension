...
from torch_geometric.datasets import Amazon, Coauthor, HeterophilousGraphDataset, WikiCS, PCQM4Mv2, ZINC
...
class GCDataset(object):
    def __init__(self, name):
        """
        Dataset class for graph classification/regression tasks.
        """
        self.name = name
        self.graphs = []
        self.labels = []
    ...
    def get_idx_split(self):
        """
        Returns the standard splits for graph-level datasets.
        """
...
def load_dataset(data_dir, dataname, sub_dataname=''):
    ...
    if dataname in  ('amazon-photo', 'amazon-computer'):
        ...
        dataset = load_zinc_dataset(data_dir)
    elif dataname == 'pcqm4mv2':
        dataset = load_pcqm4mv2_dataset(data_dir)
    else:
        raise ValueError('Invalid dataname')
    return dataset


def load_zinc_dataset(data_dir):
    transform = T.ToUndirected()
    torch_dataset = ZINC(root=f'{data_dir}/ZINC', transform=transform)

    dataset = GCDataset('zinc')
    dataset.dataset = torch_dataset  # Store the original dataset for splitting

    # Process each graph and extract necessary information
...
def load_pcqm4mv2_dataset(data_dir):
    transform = T.ToUndirected()
    torch_dataset = PygGraphPropPredDataset(name='pcqm4m-v2', root=f'{data_dir}/PCQM4Mv2', transform=transform)

    dataset = GCDataset('pcqm4mv2')
    dataset.dataset = torch_dataset  # Store the original dataset for splitting
