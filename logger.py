...
def save_model(args, model, optimizer, run):
    ...
    model_path = f'models/{args.dataset}/{args.method}_{run}_{args.beta}_{args.hidden_channels}_{args.local_layers}.pt'
...
def save_result(args, results):
    if args.dataset not in ('zinc', 'pcqm', 'ogbg-molhiv'):
        ...
        with open(f"{filename}", 'a+') as file:
            ...
            if not file_exists:
                writer.writerow(['train_roc', 'val_roc', 'test_roc', 'hidden', 'batch_size', 'lr', 'local_layers', 'global_layers', 'num_heads', 'beta', 'dropout', 'global_dropout', 'local_epochs', 'global_epochs', 'use_edges'])
            writer.writerow([round(results[0], 4), round(results[1], 4), round(results[2], 4),
                args.hidden_channels, args.batch_size, args.lr, args.local_layers, args.global_layers, args.num_heads,
                args.beta, args.dropout, args.global_dropout, args.local_epochs, args.global_epochs, args.use_edges])
